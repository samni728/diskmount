...
右上角 `PRO` 可在风险确认后显示这些高级分区。PRO 当前是**只读查看模式**：受保护设备只显示信息，不提供加载、卸载或弹出按钮；Swift 业务层也会拒绝伪造的操作请求。

## 系统要求

- Apple Silicon：M1、M2、M3、M4、M5 或后续 ARM64 Mac；
- macOS 26 或更高版本（0.1.2 使用 Tahoe 版内嵌运行时）；
- 首次初始化 NTFS microVM 数据时需要网络；
- NTFS 挂载时需要在 macOS 系统窗口中输入管理员密码。

0.1.2 不支持 Intel/x86 Mac。原因是核心 `anylinuxfs/libkrun` 上游目前只支持 Apple Silicon；App 不会把“界面能打开但核心功能不可用”描述成 Intel 支持。

## 安装与使用

1. 下载并双击 `DiskMount-0.1.2-macOS26.dmg`；
...
5. 分别签名运行时与 App；
6. 生成 `DiskMount/build/DiskMount-0.1.2-macOS26.dmg`。
